...
author: Katerin Kunfermann (12225223), Haylee Kelsall (11665424), Louelle Pesurnaij (10750886),
  Matteo Rinaldi (11580461)
...
#Stripper function used in our data work, in order to strip out numerical values from strings.
...
summarise()
RESP_DEMOG #Now we have a table with all fixed characteristics of respondents. We checked if every variables, observations, and rows were respecting the rules of tidy data. We esnured that each variable had its own column, and that values were not variables. We used the head() command to chec the first rows and then compared also to the excel tables we had. We also checked that each type of unit must have its own cell in this way, as well as the rule that each value should have its own cell. Finally, we checked and ensured that each observation had its own row: we checked the tables and we used the distinct() command to check if some of the variables were repeated.



#Here we Change W3_Q56
...
#Here we are splitting the Experiment-data
...
```{r ESS tidying chunk, include=FALSE, message = FALSE, warning = FALSE}
#We created a new table, called ESSclean, which is a table without the "fixed" characteristics (such as demographics) and the waves those variables have been measured. 
...
##The aforementioned rules about data tidying have been checked for the ESS table as well. We noticed that, creating the ESS table, some information was not stacked properly. More precisely, there were observations that were repeated in the same row. We then proceeded to create a new variable (Wave) in order to be able to stack the repeated information and give each observation its own cell (e.g. political knowledge for case 1 in wave 1, then in a different row for wave 2 etc.)

#Changing values 1 (=TRUE) to 2 (=participates in wave 2)
#Wave 1
...
#Removing WAVES and Wave columns
ESSclean$Wave <- NULL

# Cleaning up date-variables according to the new Wave variable
...
CA$date <- with(CA, dmy(sprintf('%02d%02d%02d', V3a, V3b, V3c))) #We checked in console the accuracy of our code and it returned the correct format.
#We also added unique id number to each item
CA$ID <- seq.int(nrow(CA))

#Creating table for fixed characteristics relevant to all, (i.e. items asked to everything). We checked the table correctness using console. 
ALL_FIXED <- CA %>% 
...
#We checked V6_cleaned and separated it in order to have a value in each cell, splitting the information at the ":" in the cells. We also checked the other rules for the data structure using the head() and distinct() commands and while also checking the dataframe.
ALL_FIXED <- separate(ALL_FIXED, V6_cleaned, into = c("V6Topic", "V6Topicdescription"), sep = ":")

#Creating Newspaper fixed characteristics table. We decide to include nu.nl  as a newspaper since it is treated as one in the codebook. Keeping V4 as article ID for primary key. In addition, it should be noted that note that the belgium outlets are included as they're treated as newspapers. 
CA_FIXED_NP <- CA %>%
...
#We created a new table fromt he content analysis dataset only showing television related content
CA_FIXED_TV <- CA %>%
  filter(V4 == "NOS Journaal" | V4 == "RTL Nieuws")%>% #Exclude (filter out) the newspapers
  select(V2,	V4, TV1, TV2)

#We then proceeded creating a table with all the variables coded after April the 17th
APRIL <- CA %>% 
...
filter(date >= as.Date("2014-04-17"))
...
```{r PVV50 tidying chunk, include=FALSE, message = FALSE, warning = FALSE}

#In the case of the following table, identical items were coded for PVV and 50 PLUS. Therefore, the unit would be the article and either PVV and/or 50PLUS. Although we did not tidy this table as the structure is similar to others (and it was not used by any of the group for their resulting graphs), had we tidied it, we would first have needed to create new rows for each observation, where an observation consisted of the article, and its PVV or 50PLUS item answers. The identical items would be then merged in a similar fashion to we have done with the MUS/POL table and the PolKnow example.  

#Note however - variables required by group members for their own analyses were tidied as required, rather than restructuring unused tables such as this.  
 

PVV50 <- CA %>% 
  filter(date >= as.Date("2014-04-17"))%>% #We included cases only after  the 17th of April, since the articles were coded after that period of time
...
##Katerin Kunfermann
```{r, Katerin Kunfermann, echo = FALSE, message = FALSE, warning = FALSE}
#export relevant variables into new tables KathrinCA & KathrinSurv
KathrinCA <- ALL_FIXED %>% select(V6Topic, date, V4)
KathrinSurv <- ESSclean %>% select(RESPNR, Wavevalue, date, w1_q11, w1_q12, w1_q13, w2_q8, w2_q9, w2_q10, w3_q12, w3_q13, w3_q14, w4_q14, w4_q15, w4_q16, w1_q49_1, w1_q49_2, w1_q50_1, w1_q50_2, w1_q50_6, w2_q44_1, w2_q44_2, w2_q45_1, w2_q45_2, w2_q45_6, w3_q43_1, w3_q43_2, w3_q44_1, w3_q44_2, w3_q44_6, w4_q47_1, w4_q47_2, w4_q48_1, w4_q48_2, w4_q48_6,w1_q51_2, w2_q46_2, w3_q45_2, w4_q49_2)

#Clean KathrinSurv (seperate variable names and var values)
KathrinSurv <- separate(KathrinSurv, w1_q49_1, into = c("W1Q491", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w1_q49_2, into = c("W1Q492", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w1_q50_1, into = c("W1Q501", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w1_q50_2, into = c("W1Q502", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w1_q50_6, into = c("W1Q506", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w1_q51_2, into = c("W1Q512", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q44_1, into = c("W2Q441", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q44_2, into = c("W2Q442", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q45_1, into = c("W2Q451", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q45_2, into = c("W2Q452", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q45_6, into = c("W2Q456", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q46_2, into = c("W2Q462", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q43_1, into = c("W3Q431", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q43_2, into = c("W3Q432", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q44_1, into = c("W3Q441", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q44_2, into = c("W3Q442", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q44_6, into = c("W3Q446", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q45_2, into = c("W3Q452", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q47_1, into = c("W4Q471", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q47_2, into = c("W4Q472", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q48_1, into = c("W4Q481", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q48_2, into = c("W4Q482", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q48_6, into = c("W4Q486", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q49_2, into = c("W4Q492", "varvalue"), sep = " ")

#summ all the variables up by wave
KathrinSurv$W1Q491 <- as.numeric(as.character(KathrinSurv$W1Q491))
KathrinSurv$W1Q492 <- as.numeric(as.character(KathrinSurv$W1Q492))
KathrinSurv$W1Q501 <- as.numeric(as.character(KathrinSurv$W1Q501))
KathrinSurv$W1Q502 <- as.numeric(as.character(KathrinSurv$W1Q502))
KathrinSurv$W1Q506 <- as.numeric(as.character(KathrinSurv$W1Q506))
KathrinSurv$W1Q512 <- as.numeric(as.character(KathrinSurv$W1Q512))
KathrinSurv$W2Q441 <- as.numeric(as.character(KathrinSurv$W2Q441))
KathrinSurv$W2Q442 <- as.numeric(as.character(KathrinSurv$W2Q442))
KathrinSurv$W2Q451 <- as.numeric(as.character(KathrinSurv$W2Q451))
KathrinSurv$W2Q452 <- as.numeric(as.character(KathrinSurv$W2Q452))
KathrinSurv$W2Q456 <- as.numeric(as.character(KathrinSurv$W2Q456))
KathrinSurv$W2Q462 <- as.numeric(as.character(KathrinSurv$W2Q462))
KathrinSurv$W3Q431 <- as.numeric(as.character(KathrinSurv$W3Q431))
KathrinSurv$W3Q432 <- as.numeric(as.character(KathrinSurv$W3Q432))
KathrinSurv$W3Q441 <- as.numeric(as.character(KathrinSurv$W3Q441))
KathrinSurv$W3Q442 <- as.numeric(as.character(KathrinSurv$W3Q442))
KathrinSurv$W3Q446 <- as.numeric(as.character(KathrinSurv$W3Q446))
KathrinSurv$W3Q452 <- as.numeric(as.character(KathrinSurv$W3Q452))
KathrinSurv$W4Q471 <- as.numeric(as.character(KathrinSurv$W4Q471))
KathrinSurv$W4Q472 <- as.numeric(as.character(KathrinSurv$W4Q472))
KathrinSurv$W4Q481 <- as.numeric(as.character(KathrinSurv$W4Q481))
KathrinSurv$W4Q482 <- as.numeric(as.character(KathrinSurv$W4Q482))
KathrinSurv$W4Q486 <- as.numeric(as.character(KathrinSurv$W4Q486))
KathrinSurv$W4Q492 <- as.numeric(as.character(KathrinSurv$W4Q492))

#calculate sums per wave and media usage
KathrinSurv$sum <- ifelse(KathrinSurv$Wavevalue == 1, 
                           rowSums(subset(KathrinSurv, select = c(W1Q491, W1Q492, W1Q501, W1Q502, W1Q506)), na.rm = TRUE), 
                              ifelse(KathrinSurv$Wavevalue == 2, 
                               rowSums(subset(KathrinSurv, select = c(W2Q441, W2Q442, W2Q451, W2Q452, W2Q452, W2Q462)), na.rm = TRUE), 
                                  ifelse(KathrinSurv$Wavevalue == 3, 
                                  rowSums(subset(KathrinSurv, select = c(W3Q431, W3Q432, W3Q441, W3Q442, W3Q452)), na.rm = TRUE), 
                                      ifelse(KathrinSurv$Wavevalue == 4,
                                       rowSums(subset(KathrinSurv, select = c(W4Q471, W4Q472, W4Q481, W4Q482, W4Q492)), na.rm = TRUE), 
                                                NA))))

#delete 0 value rows in media usage
KathrinSurv <- KathrinSurv %>% filter(sum >= 1)

#Clean KathrinSurv (seperate variable names and var values)
KathrinSurv <- separate(KathrinSurv, w1_q11, into = c("W1Q11", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w1_q12, into = c("W1Q12", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w1_q13, into = c("W1Q13", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q8, into = c("W2Q8", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q9, into = c("W2Q9", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w2_q10, into = c("W2Q10", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q12, into = c("W3Q12", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q13, into = c("W3Q13", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w3_q14, into = c("W3Q14", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q14, into = c("W4Q14", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q15, into = c("W4Q15", "varvalue"), sep = " ")
KathrinSurv <- separate(KathrinSurv, w4_q16, into = c("W4Q16", "varvalue"), sep = " ")

#delete varvalue
KathrinSurv$varvalue <- NULL

#set as numeric
KathrinSurv$W1Q11 <- as.numeric(as.character(KathrinSurv$W1Q11))
KathrinSurv$W1Q12 <- as.numeric(as.character(KathrinSurv$W1Q12))
KathrinSurv$W1Q13 <- as.numeric(as.character(KathrinSurv$W1Q13))
KathrinSurv$W2Q8 <- as.numeric(as.character(KathrinSurv$W2Q8))
KathrinSurv$W2Q9 <- as.numeric(as.character(KathrinSurv$W2Q9))
KathrinSurv$W2Q10 <- as.numeric(as.character(KathrinSurv$W2Q10))
KathrinSurv$W3Q12 <- as.numeric(as.character(KathrinSurv$W3Q12))
KathrinSurv$W3Q13 <- as.numeric(as.character(KathrinSurv$W3Q13))
KathrinSurv$W3Q14 <- as.numeric(as.character(KathrinSurv$W3Q14))
KathrinSurv$W4Q14 <- as.numeric(as.character(KathrinSurv$W4Q14))
KathrinSurv$W4Q15 <- as.numeric(as.character(KathrinSurv$W4Q15))
KathrinSurv$W4Q16 <- as.numeric(as.character(KathrinSurv$W4Q16))

#Calculate means per Wave
KathrinSurv$mean <- ifelse(KathrinSurv$Wavevalue == 1, 
                           rowMeans(subset(KathrinSurv, select = c(W1Q11, W1Q12, W1Q13)), na.rm = TRUE), 
                              ifelse(KathrinSurv$Wavevalue == 2, 
                               rowMeans(subset(KathrinSurv, select = c(W2Q8, W2Q9, W2Q10)), na.rm = TRUE), 
                                  ifelse(KathrinSurv$Wavevalue == 3, 
                                  rowMeans(subset(KathrinSurv, select = c(W3Q12, W3Q13, W3Q14)), na.rm = TRUE), 
                                      ifelse(KathrinSurv$Wavevalue == 4,
                                       rowMeans(subset(KathrinSurv, select = c(W4Q14, W4Q15, W4Q16)), na.rm = TRUE), 
                                                NA))))

#aggregate means per day
KathrinSurv <- KathrinSurv %>% group_by(date) %>% summarise(mean(mean))

#aggregate counts per day for economic in Content Analysis
# Filter CA to articles in Dutch media (newspapers, TV and online)
KathrinCA <- KathrinCA %>% 
  filter(V6Topic == "Economy") %>%
  filter(V4 %in% c("de Telegraaf", "de Volkskrant", "NOS Journaal", "NRC Handelsblad", "nu.nl", "RTL Nieuws")) %>% 
  count(date)

KathrinCA$n <- as.numeric(as.integer(KathrinCA$n))
Kathrinvis <- merge(KathrinSurv, KathrinCA, by = c("date"), all = TRUE)
rm(KathrinSurv)
rm(KathrinCA)

#rename columns
names(Kathrinvis) <- c("date", "econprob", "articles")
...
require(dygraphs)
require(xts)
Kathrinvisxts <-xts(Kathrinvis[,-1], order.by = as.Date(Kathrinvis$date))

dygraph(Kathrinvisxts, main = "The influence of media coverage on awareness of economic problems" ) %>%
  dySeries("articles", label = "amount of articles (economy)", fillGraph = TRUE) %>%
  dySeries("econprob", label = "awareness (economic problems)", fillGraph = TRUE) %>%
...
```
The graph shows the relationship of media coverage (offline, online, and TV) in the Netherlands and the awareness of economic problems among the participants of the four waves of the survey over time. As shown, the amount of the media coverage does not have an influence on the awareness of economic problems.
The absence of the first level agenda-setting effect (stating that the higher media coverage about a certain topic the higher the awareness among participants about a certain topic) can be explained by a possible influence of other variables that influence the relationship (such as e.g. length of article/TV Report). Additionally, the four-wave-structure of the survey restrict the ability to generalize statements since the gaps in-between the waves set boundaries to make generalized statements.

## Louelle Pesurnaij
### European Election Campaign Study 2014  (Panel Survey & Media Content Analysis)
...
In the graph above, the relationship between Dutch media coverage (both newspaper, TV and online) and political cynicism amongst the public (from 1 -totally disagree- to 7 -totally agree-) is plotted over time. The graph shows that the amount of Dutch articles about the 2014 EU Elections does not have a direct influence on political cynicism amongst the public. This contradicts the second-level agenda setting theory, which implies that media influence how the public perceives certain topics (such as politics). 

When critically assessing the graph, some uncertainty problems arise. First of all, political cynicism has only been measured in the four-wave panel survey, thus, political cynicism is not continously measured over time. Therefore it cannot be concluded with full confidence that there is an absence of the (second-level) agenda setting effect. Besides, article-attributes (e.g. the content and sentiment) are not taken into account within this analysis and it also remains unclear whether the survey-respondets were actually exposed to the media content or not. These two factors might play an important (moderating) role in the influence of media coverage on political cynicism. In future research, these factors could be taken into account.

## Matteo Rinaldi
### Political Knowledge and Anti-Immigrant attitudes: what relationship?
```{r Matteo Rinaldi, echo = FALSE, message = FALSE, warning = FALSE}
#Using the stripper function in order to treat data about anti-immigration attitudes as numeric and be able to use only the numerical number from the observations (E.g. '1 helemaal mee eens' becomes 1).Adding columns for the anti-immigration attitudes as umeric and then recode them as 1 when the attitudes is 5 or higher towards a negative attitude.
MAT <- ESSclean %>%
...
In this graphical representation I am using the data from the survey conducted between December 2013 and May 2014 about the attitudes and intentions of citizens towards the European Union. I decided to analyse the interaction between the political knowledge of the surveyd sample and their attitudes towards immigration. I was expecting this relation to be negative: the more one knows about politics and its complexity, the more should be aware that immigration is a complex process and that immigrants are not to blame for the core of the problem itself. I was then expecting to find a higher count of anti-immigrant attitudes scores for those people who scored lower in political knowledge. I conducted the analysis by recoding the measurement for political knowledge as a dummy variable (right/wrong, coded as 1/0) and by recoding the variables measuring anti-immigrant attitudes (properly inverse recoded when needed) by deciding that cases scoring higher or equal to 5 (on a 1 - 7 Likert scale) had to be considered like having a negative attitude toward immigrants, being recoded then with a 1. 
I then created two new variables which are the mean of these results for each participant. 
I proceeded displaying how the interaction took place with a visualization with ggplot. Interestingly, it seems like the people knowing average about politics (in the centre of the 2d table) are the ones having both the most extreme and most tolerant attitudes towards immigrants. The situation did not changed with the second wave. Finally, as expected, people low in political interest, or with an amount below the 50% of right scores, are the ones that scored lower on anti-immigrant attitudes, showing then lower tolerance and levels of acceptace of immigrants in the country and in the Dutch culture. 



<center> <h1> Do readers reflect the attitudes of their daily newspaper? </h1> </center>
<center> <h3> __Dutch newspapers, their readers, and their European attitudes__ </h3> </center>

In 1972, McCombs and Shaw published their seminal article, _"The agenda-setting function of mass media"_. They argued that mass media - through their ability to increase the salience of a given issue in the public eye - are able to set the public's agenda. In 1984, Entman introduced _"Framing"_, which extended agenda-setting by highlighting that the press, by selecting _particluar_ aspects of an issue, promte a particular understanding, evaluation or course of action with regards the given issue. Can we see this in the case of Dutch newspaper readers reflecting similar European attitudes to those which are presented in the daily paper which they read? 
The interactive graph below plots the mean monthly EU-attitudes score of readers versus the EU-attitudes presented in the newspaper they read (either de Telegraaf, NRC Handelsblad or de Volkskraant). The individual dots represent unique readers' EU-attitudes by month - an average score based on 18 items. Interestingly, individuals consistently held more pro-European attitudes than the newspapers they read, and for each newspaper, we see individual cases which deviate from the rest of the group, such as the two lone Telegraaf readers in March with _very_ pro-EU attitudes. Unfortunately it is difficult to conclude whether indeed reader attitudes are influenced by those in the press, or whether it is that readers opt for news outlets which reflect their pre-existing beliefs. However, the slopes of the lines do suggest there may be a pattern between reader attitudes and those in their respective paper, and looking at this over a longer period may provide confirmation that indeed such a relationship exists. For now however what is clear is that in the Netherlands, citizens' appear to be far more supportive of the European Project than the daily newspapers which they read. 


```{r Haylee chunk, echo = FALSE, warning = FALSE, message = FALSE}
